...
// Create database
if(!mysql_query("exists test" ))
	{
	mysql_query("CREATE DATABASE test",$conn);
	// Create table
...
switch($method){
	...
	case 'delete':
		...
		echo '<div  class="alert alert-success">'.$textd. '<a href="' . $php_file . '">' . " 返回首页" . '</a></div>';
		mysql_close($conn);
		break;
	case 'save':
		//保存数据提交，并且验证是POST提交
		if(isset($_POST['submit'])) {
			...
			if($postid) {
				//编辑
				$result = mysql_query("UPDATE `domain` SET `domain`='$domain',`name`='$name',`nickname`='$nickname',`ip`='$ip',`ftphost`='$ftphost',`ftpuser`='$ftpuser', ftppass='ftppass',`edittime`='$edittime', note='$note' WHERE `postid`=$postid", $conn);
				$text = '更新成功';
			} else {
				//添加
				$result = mysql_query("INSERT INTO domain (domain,name,nickname,ip,ftphost,ftpuser,ftppass,datetime,edittime,note) VALUES ('$domain','$name','$nickname','$ip','$ftphost','$ftpuser','$ftppass','$datetime','$edittime','$note')", $conn);
				$text = '添加成功' ;
			}
			
			if($result)
				echo '<div  class="alert alert-success">'.$text. '<a href="' . $php_file . '">' . " 返回首页" . '</a></div>';
		}
		mysql_close($conn);
		break;
	default:
